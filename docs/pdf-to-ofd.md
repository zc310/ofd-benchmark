...
|----------------|----------:|----------:|----------:|----------:|---------:|---------------:|-------------------:|---------:|
| go-zc310      |    43ms  |    89ms  |    67ms  |    1.12s  |    333ms  |     1.96s  |    3.68s | 0 |
| go-ofdgo      |  FAILED |  FAILED |  FAILED |  FAILED |  FAILED |  FAILED |  15.24s | 0 |
| rust-easyofd  |  **2.6ms** |  **3.6ms** |  **2.6ms** |   **104ms** |     **23ms** |    **277ms**  |  **86.7ms** | 7 |
| java-ofdrw     |     496ms |     1.25s |     1.04s |     4.40s |    2.92s |         26.09s |  12.65s |        0 |
| python-pdf2ofd |     106ms |     817ms |     531ms |     2.43s |    943ms |         34.49s |  18.34s |        0 |
...
| go-ofdgo      |  FAILED |  FAILED |  FAILED |  FAILED |  FAILED |  FAILED |   — | 0 |
...
| go-ofdgo      |  FAILED   |  FAILED   |  FAILED   |  FAILED    |  FAILED   |  FAILED           |   —                       |
...
> - go-ofdgo 7 个文件仅 GBT_33190-2016 转换成功（字体全内嵌），其余 6 个失败：非 Identity-H 复合字体编码 hello/999/zsbk/1000-pages、Stamp 注解 ano、intro 运行时 nil 指针 panic（较上一版从「非嵌入字体/字体数据解析」报错变为 panic，非嵌入字体问题已修复）；且其输出 OFD 写 16 位色值（`Value="0 0 0 65535"`），zc310 读取器拒绝解析（color value out of range 0-255），故 GBT 的像素/文本无法回环测量（—），文本实际保留在 XML 中（TextCode 约 24 万字符）
> - 像素相似度基于整页平均色差，对内容稀疏的页面（白底小字）区分度有限，需结合文本提取与人工核对判断
> - 回环对比引入了 go-zc310 的 OFD→PDF 渲染质量，数据仅供参考

## 结论

- **go-zc310**：唯一保留可复制文本（顺序正确）且可用的转换器，像素还原 2 胜（ano、zsbk），在可用转换器中速度/体积最优（43ms~3.68s、1.6K~6.6M）
- **go-ofdgo**：能力设计最全（路径/文本/图片/注解全量导入），7 个文件仅 GBT_33190-2016（全内嵌字体）成功——其余 6 个失败（4 个非 Identity-H 字体编码、1 个 Stamp 注解、intro nil 指针 panic），且输出 OFD 用 16 位色值，通用读取器无法解析，互操作性差
