...
| go-ofdgo       | Go     | [xiaoqidun/ofdgo](https://github.com/xiaoqidun/ofdgo)（ConvertPDF + pdfgo）                | 2026-09-25 版          | 完整导入（路径/文本/图片/注解），7/7 全部成功（已修复非 Identity-H 字体编码、Stamp 注解、nil panic） |
...
> go-ofdgo 自 2026-09-25 15:21 版起 7/7 全部成功（此前非 Identity-H 编码/Stamp 注解硬失败）
...
| go-ofdgo      |  99.95% |  99.41% |  98.27% |  **99.80%** |  99.12% |  99.45% |  99.42% | 1 |
...
> - go-ofdgo 7/7 全部成功（2026-09-25 15:21 版修复非 Identity-H 复合字体编码、Stamp 注解与 nil panic）；intro 像素 99.80% 全场最高；文本提取量普遍多于 zc310（hello 41、1000-pages 534998、GBT 234779 字符）但 4-gram 相似度明显偏低（顺序/字距差异，GBT 仅 1.08%）
...
- **go-ofdgo**：能力设计最全（路径/文本/图片/注解全量导入），7/7 全部成功——intro 像素 99.80% 全场最高、hello 速度反超 zc310（13.7ms）；文本提取量最大（1000-pages 534998、GBT 234779 字符）但相似度偏低（≤29.63%）
