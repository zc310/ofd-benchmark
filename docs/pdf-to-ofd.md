...
| go-ofdgo       | Go     | [xiaoqidun/ofdgo](https://github.com/xiaoqidun/ofdgo)（ConvertPDF + pdfgo）                | 2026-09-25 版          | 完整导入（路径/文本/图片/注解），非嵌入字体已支持，非 Identity-H 字体编码/Stamp 注解仍硬失败，本测试集 2/7（intro、GBT 成功） |
...
| go-ofdgo      |  FAILED |  FAILED |  FAILED |  2.40s |  FAILED |  FAILED |  15.17s | 0 |
...
> go-ofdgo 仅 intro、GBT_33190-2016 成功，其余 5 个失败（详见「还原度对比」注释）
...
| go-ofdgo      |  FAILED |  FAILED |  FAILED |  8.5M |  FAILED |  FAILED |  12.2M | 0 |
...
| go-ofdgo      |  FAILED |  FAILED |  FAILED |  1.24 |  FAILED |  FAILED |  2.47 |
...
| go-ofdgo      |  FAILED |  FAILED |  FAILED |  **99.80%** |  FAILED |  FAILED |  99.42% | 1 |
| rust-easyofd  |  99.84% |  98.56% |  95.77% |  43.71% |  95.62% |  73.67% |  96.28% | 0 |
| java-ofdrw     | **99.98%** |     98.37% |     98.72% | 99.34% |     99.22% |     **99.62%** | **99.62%** |        3 |
...
| go-ofdgo      |  FAILED   |  FAILED   |  FAILED   |  10927     |  FAILED   |  FAILED           |       234779               |
...
| go-ofdgo      |  FAILED |  FAILED |  FAILED |  13.71% |  FAILED |  FAILED |   1.08% |
...
> - go-ofdgo 2/7 成功（intro、GBT_33190-2016，字体内嵌），其余 5 个失败：非 Identity-H 复合字体编码 hello/999/zsbk/1000-pages、Stamp 注解 ano；intro 像素 99.80% 为全场最高（超过 ofdrw），文本可提取（intro 10927、GBT 234779 字符）但 4-gram 相似度低（13.71%、1.08%）；输出 OFD 的 16 位色值新版 go-zc310 已兼容，nil 指针 panic 已修复（intro 本版成功）
...
- **go-ofdgo**：能力设计最全（路径/文本/图片/注解全量导入），2/7 成功（intro、GBT）——intro 像素 99.80% 全场最高、GBT 99.42%，文本可提取但相似度低（13.71%、1.08%）；其余 5 个失败（4 个非 Identity-H 字体编码、1 个 Stamp 注解）
...
- **java-ofdrw**：像素还原 3 胜（hello、1000-pages、GBT，外观贴近源 PDF），文本转为轮廓不可提取；输出体积大（1000-pages 57MB、GBT 30.5MB）、多页转换慢（GBT 12.78s）
